# -*- coding: utf-8 -*-
"""
Created on Fri Oct 12 14:46:15 2018
...
def build_model(
    ...
    model_detector=keras.applications.inception_resnet_v2.InceptionResNetV2(include_top=False, weights='imagenet', input_tensor=tensor_input, input_shape=(299,299,3), pooling='avg')
        ### 
    ### bi-linear pooling
    ###
...
def train_model(
    ...
    checkpoint = ModelCheckpoint('weights.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='min', period=1)
    learning_rate_reduction = ReduceLROnPlateau(monitor='val_acc', patience=1, verbose=1, factor=0.5, min_lr=0.0000001)
    ...
    hist = model.fit_generator(
                gen_dir_train
                ,samples_per_epoch=len(train_labels)//20
                ,nb_epoch=max_epoch
                , validation_data=gen_dir_valid
                ,nb_val_samples=len(train_labels1)//20
